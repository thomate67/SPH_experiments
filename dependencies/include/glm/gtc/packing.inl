...
namespace detail
{
    ...
    //	{
    //		return ((floatTo11bit(x) & ((1 << 11) - 1)) << 0) |  ((floatTo11bit(y) & ((1 << 11) - 1))
    //<< 11) | ((floatTo10bit(z) & ((1 << 10) - 1)) << 22);
    //	}
    ...
    struct compute_half<3, Q>
    {
        ...
        GLM_FUNC_QUALIFIER static vec<3, float, Q> unpack(vec<3, uint16, Q> const& v)
        {
            i16vec3 Unpack;
            memcpy(&Unpack, &v, sizeof(Unpack));
            return vec<3, float, Q>(
                    detail::toFloat32(v.x), detail::toFloat32(v.y), detail::toFloat32(v.z));
        }
    };
...
GLM_FUNC_QUALIFIER uint32 packF3x9_E1x5(vec3 const& v)
{
    ...
    float const ExpShared  = detail::compute_equal<float>::call(MaxShared, pow(2.0f, 9.0f))
                                    ? ExpSharedP + 1.0f
                                    : ExpSharedP;
